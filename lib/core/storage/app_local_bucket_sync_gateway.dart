...
import 'app_local_bucket_store.dart';

const appLocalBucketSyncProtocolVersion = '1.0.0';

/// 为当前 App WebView 提供阻塞 XHR 可用的回环 JSON 通道。
///
/// Gateway 绑定随机 loopback 端口和 256-bit capability path。它只访问注入的
/// [AppLocalBucketStore]，不会代理到 Authority 或对局网络。
final class AppLocalBucketSyncGateway {
  AppLocalBucketSyncGateway._(this._server, this._store, this._capability);

  static const _maxRequestBytes =
      AppLocalBucketStore.maxBucketJsonBytes + 128 * 1024;
  static final _requestIdPattern = RegExp(
    r'^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$',
  );

  final HttpServer _server;
  final AppLocalBucketStore _store;
  final String _capability;
  final Map<String, _AppLocalBucketSyncLedgerEntry> _ledger = {};
  bool _closed = false;

  static Future<AppLocalBucketSyncGateway> start(
    AppLocalBucketStore store,
  ) async {
    ...
    final capability = base64Url
        ...
        .replaceAll('=', '');
    final gateway = AppLocalBucketSyncGateway._(server, store, capability);
    gateway._listen();
    return gateway;
  }
  ...
  Future<void> _handle(HttpRequest request) async {
    ...
    if (envelope['protocolVersion'] != appLocalBucketSyncProtocolVersion) {
  ...
  Future<Object?> _runOnce({
    ...
    final result = action();
    _ledger[requestId] = _AppLocalBucketSyncLedgerEntry(digest, result);
...
final class _AppLocalBucketSyncLedgerEntry {
  const _AppLocalBucketSyncLedgerEntry(this.digest, this.result);
