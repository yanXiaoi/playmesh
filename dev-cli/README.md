...
把生成的可执行文件放入 `PATH` 后即可在任意项目目录调用。CLI 当前开发版本为 `1.4.0`（正式发布基线为 `1.1.0`），可通过 `playmesh-cli --version`、`playmesh-cli -v` 或 `playmesh-cli version` 查看；正式二进制名称固定为 Windows 的 `playmesh-cli.exe` 和 macOS/Linux 的 `playmesh-cli`。
...
- `playmesh-cli push`：先按本地 SDK 文件重写 `main.json.sdkVersion/appSdkVersion`，
  并按当前 Manifest 已知字段统一投影（所有普通多余字段都不写回），再只上传
  `main.json`、可选根
  `icon.png`、`capabilities.json` 和 `app/`，校验并原子提交，不启动游戏。无效或
  超过 2 MiB 的图片会被忽略。
...
```text
main.json
icon.png             # 可选游戏列表图标；PNG，最大 2 MiB
capabilities.json
...
目标 App 继续复用正式游戏包导入器完成 ZIP 路径安全、文件数/大小、Manifest、能力、根 `icon.png` 和入口校验。相同 `main.json.id` 表示更新：只替换 `main.json`、可选 `icon.png`、`capabilities.json` 与 `app/`，目标中的 `data/`、`cache/` 及其他运行内容保持原样；远端项目没有有效图标时 `get/create` 会删除本地旧图标，避免继续发布陈旧图片。失败时恢复原发布文件。不同 ID 表示新增。
