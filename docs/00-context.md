...
决定玩法开始和结束条件，SDK 只提交受控会话状态请求。主 App Android/Windows 正式包、
Runtime 三端底包与 WebIDE 开发包均已从该基线构建并通过自动门禁；两套 SDK 版本不变。工程
落点记录在
...
- 游戏包导出不再出现在游戏详情；源代码开发工作区的“导出”统一提供“导出源码”、
  “上传到发布源”和“安装包导出”。源码包与安装包在 App WebView 中共用原生宿主保存，
  在普通浏览器中直接下载；安装包按目标复用或按 `App.json` Runtime 清单下载底包，主 App
  不内置具体 Runtime。
...
- App/Runtime 首次创建本地资料时，同时生成随机昵称和 `u_` 前缀的 UUID v4，并在本次安装且应用数据未清除期间保持稳定。换设备、重装或清理应用数据后允许生成新 ID；昵称仍可修改，不作为身份主键。
