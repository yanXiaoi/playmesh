...
- 本地导入、CLI 开发和 App 运行不对 HTML 查询参数做 `http` 等内容策略限制，只按
  URI 语法解析并透传。上传到 go-server 云分发时，外部 HTTP/WS、协议相对链接、动态
  `Function`、`file:`、`javascript:` 及 `iframe/object/embed` 不作为默认拒绝条件；
  可执行 HTML Data URL、`eval`、Service Worker 和活动 SVG 等其余启用规则仍可拒绝包。
- Go Server 接收时不对 `app/` 内其他资源使用扩展名或文件类型白名单；它只从
  `main.json.entries.game` 取得首页路径，去掉可选查询串后要求其指向 `.html`，并确认
...
游戏库在 App 启动、恢复、导入或删除后扫描 `packages/`。游戏库页面右上角提供“重新扫描游戏库”按钮，新增游戏后不要求重启 App。扫描在后台执行：当前列表继续使用旧缓存，扫描成功后才原子替换。单个项目只要 `main.json` 能解析出符合上述共享规则的 `id`，即使其他字段或入口损坏，也必须以“待修复”状态保留，不能阻断 App 或开发者工作区；运行与正式导入仍执行完整校验，源码工作区的“上传到发布源”则使用宽松导出，不执行项目语义校验。可恢复扫描只保留原始 `remarks` 和机器状态，不向游戏简介、显示模式或状态字段写入本地化文案；当 `remarks` 为空时，由 App UI 使用统一语言包显示修复说明。无法解析 JSON 或没有合法 `id` 的目录只记录诊断并跳过。
