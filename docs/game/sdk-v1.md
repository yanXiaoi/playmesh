...
统一菜单点击“分享/邀请”后只让该按钮进入加载态，其他菜单项不锁定；宿主分享层先完成
首帧显示，再在局域网内容区域使用既有加载态建立分享通道，不能用整页加载遮罩阻塞反馈。
减少动态效果偏好下按钮加载环停止旋转。
...
和普通浏览器都没有该入口。发现局域网房间期间只在房间列表区域显示“扫描中”动效并阻止
列表交互，扫码按钮和邀请链接输入保持可用；减少动态效果偏好下停止旋转动画。
