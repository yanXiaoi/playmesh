...
- `ohos/AppScope/app.json5`：`versionName=1.6.3`、`versionCode=11`
...
- App、OpenHarmony、更新日志、开发文档、提示词、SDK 生成物和测试版本已对齐。
...
| 命令 | 结果 |
|---|---|
| `flutter analyze --no-pub` | 仅命中 `build/harmony-release` 隔离副本缺少 Windows WebView 依赖的 8 个既有错误和鸿蒙兼容层 3 条风格提示 |
...
全量 `flutter analyze` 会递归进入为 OpenHarmony 构建生成的 `build/harmony-release` 工作副本；该副本刻意排除 Windows WebView 包，不能代表 `lib/` 与 `test/` 源目录分析结果。Android 构建另提示 `mobile_scanner` 仍应用 Kotlin Gradle Plugin，需在未来 Flutter 强制 Built-in Kotlin 前升级依赖。
...
- 在 Android、iOS 和 HarmonyOS 真机声明 `device.vibration`，分别测试五种 style 的实际触感与系统设置影响。
