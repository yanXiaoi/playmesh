...
允许插件在真实创建失败后调用平台诊断桥细分失败原因，但诊断不能在创建之前短路真实
初始化，也不能影响 bootstrap、能力清单或同步 `isAvailable`。
...
创建或释放抛出任何其他异常
  => status = failed
  => CapabilityOperationException 同时返回稳定 errorCode
```

自检结果不把能力加入游戏声明；它使用和游戏相同的真实 `create()` 路径，因此可能
触发该能力正常创建时需要的系统授权。实例创建成功后必须立即释放。插件不得把平台
诊断桥包装成“自检成功”，也不得因诊断失败而跳过已经约定的真实创建尝试。
