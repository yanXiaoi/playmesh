...
- 项目校验不从 HTML/CSS/JavaScript 文本静态推断本地资源是否存在；这类失败由真实运行时
  资源请求直接反馈，避免误拒动态 `import()`、模板路径和构建器生成资源。静态扫描仍拒绝
  越出 `app/` Web 根目录的引用；`main.json` 明确声明的游戏、控制器和 Authority 入口仍必须存在。
