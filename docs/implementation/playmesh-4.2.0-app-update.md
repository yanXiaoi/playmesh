...
打包资源只有 `assets/app/App.json`。`assets/app/app_update.json` 只是远端动态数据格式示例，
不进入 Flutter 资源清单，也不作为运行时回退数据。
...
设置页“检查更新”
  ...
  -> NamedDownloadEndpointList 严格解析清单源
...
平台没有下载信息；不会退回较旧但有当前平台下载的清单。同版本按 `App.json` 中的原始
顺序稳定选择第一个有效源。

## 数据模型与校验

- `App.json` 是最多 16 项的 `{name,url}` 数组；名称、URL 不允许重复。
- 清单源和下载线路只接受无凭据、无 Fragment 的 canonical HTTPS URL。
...
- 内置 `App.json` 无效与“全部远端清单无效或不可达”使用不同错误状态。
