...
- 源码导出复用既有宽松 `/package` Operation；App WebView 与 GDevelop 共用同一原生
  保存钩子和宿主流式下载服务，普通浏览器直接下载。上传分支保留既有完整校验、多源上传、
  状态和失败重试语义。安装包分支使用独立 `package_exports.*` Operation；主 App 不携带
...
分享功能必须复用游戏包服务、版本记录、权限提示和安全边界。宽松导出只用于备份与修复，
不能伪装成校验成功；上传到发布源、重新导入与安装仍不得绕过 `main.json` 校验、压缩包
路径检查、恶意资源限制或用户确认流程。
