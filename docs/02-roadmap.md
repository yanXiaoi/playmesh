...
- 游戏详情不再提供游戏包导出。源代码开发工作区将原“发布”入口改为“导出”，先选择
  ...
  Android x86_64 或 Windows x64，并可配置最多一个内置中转服务器。
- 源码导出复用既有宽松 `/package` Operation；App WebView 与 GDevelop 共用同一原生
  保存钩子和宿主流式下载服务，普通浏览器直接下载。上传分支保留多源上传、状态和失败重试
  语义，但当前不再执行项目校验；项目校验 Operation、既有 AI 提示、本地运行和重新导入
  的校验要求不变。
  安装包分支使用独立 `package_exports.*` Operation；主 App 不携带
  Runtime 底包，底包按 `App.json → Runtime update.json → 目标下载线路` 获取、SHA-256
  校验并保存到固定游戏库目录，已有文件默认复用且可强制重新下载，测试期可手工复制模拟。
  安装包与源码包共用浏览器下载/App 原生保存函数，并显示 Runtime 下载和原生导出进度。
  Android、iOS、Windows 的实际系统交付界面仍待对应设备验收。
...
- 通过局域网地址分享游戏包或当前游戏入口。
- 浏览器多人入口没有有效本地昵称时自动生成并保存“浏览器”加 4 位随机字母或数字，不再要求首次输入；已有昵称、手动改名和同 ID 重连继续保留。
- 链接必须携带必要的游戏 ID、版本或包标识和临时 token，并由 Go Core/App 在接收端进行校验。
- 链接在当前游戏会话期间有效。关闭分享界面、浏览器刷新和 App 刷新游戏都不撤销 token；离开游戏、会话结束或 Core 重启后立即失效。
- 同一会话重新打开分享界面或刷新游戏都复用原 token；只有退出并创建新会话才生成新 token。
...
- 源代码开发工作区是文件导出的唯一产品入口，游戏详情不重复显示导出操作。
- 普通浏览器使用标准下载；App WebView 由原生宿主显示系统保存或分享界面，并从同源
  Developer Gateway 流式接收项目包。
- 导出继续复用现有 `/package` 宽松备份语义，允许取回待修复项目；包中只包含发布内容，
  不包含运行数据、缓存或开发历史。接收端仍需经过严格包校验和安装流程。

分享功能必须复用游戏包服务、版本记录、权限提示和安全边界。宽松导出不能伪装成校验
成功；源码工作区上传到发布源不执行项目语义校验，但仍保留生成上传请求所需的基础
Manifest 元数据、压缩包路径和容量边界。重新导入与安装继续执行完整校验及用户确认。
