...
AI 提示词模板按 locale 放在
`assets/playmesh-library/public/developer/prompts/{locale}/`。可用语言、默认语言和
fallback 统一来自全局语言清单，动态提示词界面文案来自对应的全局 `app.json`；新增
提示词语言只增加资源和清单配置，不增加特定语言的 Dart 或 JavaScript 分支。
